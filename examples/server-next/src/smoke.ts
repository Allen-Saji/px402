/**
 * Smoke driver for @px402/next. Boots `next dev`, hits the gated route via
 * @px402/client, asserts a 200. Exits non-zero on failure.
 */
...
function bootServer(serverPubkey: string, port: number): { child: ChildProcess; ready: Promise<void> } {
  ...
  let buf = "";
  child.stdout?.on("data", (d) => { buf += d.toString(); process.stdout.write(`[next] ${d}`); });
  child.stderr?.on("data", (d) => { buf += d.toString(); process.stderr.write(`[next-err] ${d}`); });
  const ready = new Promise<void>((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error("next dev boot timeout 60s")), 60000);
    const watcher = setInterval(() => {
      if (/Ready in|started server on/i.test(buf)) {
        clearTimeout(timer);
      ...
      if (child.exitCode !== null) {
        clearTimeout(timer);
        clearInterval(watcher);
        reject(new Error(`next dev exited with ${child.exitCode} before ready`));
      }
    }, 250);
  });
  return { child, ready };
...
async function main() {
  ...
  const port = 8903 + Math.floor(Math.random() * 100);
  const { child, ready } = bootServer(server.publicKey.toBase58(), port);
  try {
    await ready;
    log(`next ready on :${port}, giving subscriber 8s head start (route handler hoists subscriber on first hit)`);
    // Hit the index page once so the route module loads + subscriber starts.
    ...
    if (!body || (body as { source?: string }).source !== "next") {
